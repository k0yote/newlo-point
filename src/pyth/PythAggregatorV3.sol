...
contract PythAggregatorV3 {
    ...
    function latestAnswer() public view virtual returns (int) {
        PythStructs.Price memory price = pyth.getPriceUnsafe(priceId);
        return int(price.price);
    }

    function latestTimestamp() public view returns (uint) {
        PythStructs.Price memory price = pyth.getPriceUnsafe(priceId);
    ...
    function getAnswer(uint) public view returns (int) {
        return latestAnswer();
    }
    ...
    {
        PythStructs.Price memory price = pyth.getPriceUnsafe(priceId);
        return (_roundId, int(price.price), price.publishTime, price.publishTime, _roundId);
    }
        ...
        returns (uint80 roundId, int answer, uint startedAt, uint updatedAt, uint80 answeredInRound)
    {
        ...
        return (roundId, int(price.price), price.publishTime, price.publishTime, roundId);
    }
}
